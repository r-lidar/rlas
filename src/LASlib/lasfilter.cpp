...
BOOL LASfilter::parse(int argc, char* argv[])
{
  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strncmp(argv[i],"-clip_", 6) == 0)
    {
      ...
      {
        REprintf("WARNING: '%s' will not be supported in the future. check documentation with '-h'.\n", argv[i]);
    ...
    else if (strncmp(argv[i],"-keep_", 6) == 0)
    {
        ...
        if (strcmp(argv[i],"-keep_classification") == 0 || strcmp(argv[i],"-keep_class") == 0)
        {
          ...
          *argv[i]='\0';
          ...
          do
          {
            if (atoi(argv[i]) > 31)
            {
              REprintf("ERROR: cannot keep classification %d because it is larger than 31\n", atoi(argv[i]));
              return FALSE;
            }
            else if (atoi(argv[i]) < 0)
            {
              REprintf("ERROR: cannot keep classification %d because it is smaller than 0\n", atoi(argv[i]));
              return FALSE;
            }
        ...
        if (strcmp(argv[i],"-keep_extended_classification") == 0 || strcmp(argv[i],"-keep_extended_class") == 0)
        {
          ...
          *argv[i]='\0';
          ...
          do
          {
            if (atoi(argv[i]) > 255)
            {
              REprintf("ERROR: cannot keep extended classification %d because it is larger than 255\n", atoi(argv[i]));
              return FALSE;
            }
            else if (atoi(argv[i]) < 0)
            {
              REprintf("ERROR: cannot keep extended classification %d because it is smaller than 0\n", atoi(argv[i]));
              return FALSE;
            }
      ...
      else if (strncmp(argv[i],"-keep_user_data", 15) == 0)
      {
        ...
        else if (strcmp(argv[i],"-keep_user_data_between") == 0)
        {
          if ((i+2) >= argc)
          {
            REprintf("ERROR: '%s' needs 2 arguments: min_value max_value\n", argv[i]);
            return FALSE;
          }
      ...
      else if (strncmp(argv[i],"-keep_point_source", 18) == 0)
      {
        if (strcmp(argv[i],"-keep_point_source") == 0)
        {
          if ((i+1) >= argc)
          {
            REprintf("ERROR: '%s' needs 1 argument: ID\n", argv[i]);
            return FALSE;
          }
    ...
    else if (strncmp(argv[i],"-drop_", 6) == 0)
    {
        ...
        if (strcmp(argv[i],"-drop_classification") == 0 || strcmp(argv[i],"-drop_class") == 0)
        {
          ...
          *argv[i]='\0';
          ...
          do
          {
            if (atoi(argv[i]) > 31)
            {
              REprintf("ERROR: cannot drop classification %d because it is larger than 31\n", atoi(argv[i]));
              return FALSE;
            }
            else if (atoi(argv[i]) < 0)
            {
              REprintf("ERROR: cannot drop classification %d because it is smaller than 0\n", atoi(argv[i]));
              return FALSE;
            }
        ...
        if (strcmp(argv[i],"-drop_extended_classification") == 0 || strcmp(argv[i],"-drop_extended_class") == 0)
        {
          ...
          *argv[i]='\0';
          ...
          do
          {
            if (atoi(argv[i]) > 255)
            {
              REprintf("ERROR: cannot drop extended classification %d because it is larger than 255\n", atoi(argv[i]));
              return FALSE;
            }
            else if (atoi(argv[i]) < 0)
            {
              REprintf("ERROR: cannot drop extended classification %d because it is smaller than 0\n", atoi(argv[i]));
              return FALSE;
            }
      ...
      else if (strncmp(argv[i],"-drop_user_data", 15) == 0)
      {
        ...
        else if (strcmp(argv[i],"-drop_user_data_below") == 0)
        {
          if ((i+1) >= argc)
          {
            REprintf("ERROR: '%s' needs 1 argument: min_value\n", argv[i]);
            return FALSE;
          }
        ...
        else if (strcmp(argv[i],"-drop_user_data_above") == 0)
        {
          if ((i+1) >= argc)
          {
            REprintf("ERROR: '%s' needs 1 argument: max_value\n", argv[i]);
            return FALSE;
          }
    ...
    else if (strncmp(argv[i],"-thin_", 6) == 0)
    {
      if (strcmp(argv[i],"-thin_with_grid") == 0)
      {
        if ((i+1) >= argc)
        {
          REprintf("ERROR: '%s' needs 1 argument: grid_spacing\n", argv[i]);
          return FALSE;
        }
      ...
      else if (strcmp(argv[i],"-thin_pulses_with_time") == 0 || strcmp(argv[i],"-thin_with_time") == 0)
      {
        if ((i+1) >= argc)
        {
          REprintf("ERROR: '%s' needs 1 argument: time_spacing\n", argv[i]);
          return FALSE;
        }
  ...
  if (drop_return_mask)
  {
    if (keep_return_mask)
    {
      REprintf("ERROR: cannot use '-drop_return' and '-keep_return' simultaneously\n");
      return FALSE;
    }
    else
    {
  ...
  if (keep_classification_mask)
  {
    ...
    {
      REprintf("ERROR: cannot use '-drop_class' and '-keep_class' simultaneously\n");
      return FALSE;
    }
    else
    {
