...
BOOL LASreaderTXT::open(FILE* file, const char* file_name, const char* parse_string, I32 skip_lines, BOOL populate_header)
{
  ...
  if (number_attributes)
  {
    for (i = 0; i < number_attributes; i++)
    {
      ...
      try {
        ...
        throw std::runtime_error(std::string("ERROR: initializing attribute %s")); //attribute_descriptions[i]
        return FALSE;
      }
    }
  }
  ...
  if (populate_header && file_name)
  {
    ...
    while (fgets(line, 512, file))
    {
      ...
      else
      {
        ...
        throw std::runtime_error(std::string("WARNING: cannot parse '%s' with '%s'. skipping ...")); //line, parse_less
      }
    }
    ...
    if (npoints == 0)
    {
      throw std::runtime_error(std::string("ERROR: could not parse any lines with '%s'")); //parse_less
      fclose(file);
      file = 0;
      free(parse_less);
      return FALSE;
    ...
    if (file == 0)
    {
      throw std::runtime_error(std::string("ERROR: could not open '%s' for second pass")); //file_name
      return FALSE;
    }
  ...
  while (fgets(line, 512, file))
  {
    ...
    {
      line[strlen(line)-1] = '\0';
      throw std::runtime_error(std::string("WARNING: cannot parse '%s' with '%s'. skipping ...")); //line, this->parse_string
    }
  }
  ...
  if (i != 1)
  {
    throw std::runtime_error(std::string("ERROR: could not parse any lines with '%s'")); //this->parse_string
    fclose(this->file);
    this->file = 0;
    ...
    while (true)
    {
      ...
      else
      {
        if (populated_header)
        {
          ...
          {
#ifdef _WIN32
            throw std::runtime_error(std::string("WARNING: end-of-file after %I64d of %I64d points")); //p_count, npoints
#else
            throw std::runtime_error(std::string("WARNING: end-of-file after %lld of %lld points")); //p_count, npoints
#endif
          }
        }
        else
...
BOOL LASreaderTXT::reopen(const char* file_name)
{
  ...
  while (fgets(line, 512, file))
  {
    ...
    {
      line[strlen(line)-1] = '\0';
      throw std::runtime_error(std::string("WARNING: cannot parse '%s' with '%s'. skipping ...")); //line, parse_string
    }
  }
  ...
  if (i != 1)
  {
    throw std::runtime_error(std::string("ERROR: could not parse any lines with '%s'")); //parse_string
    fclose(file);
    file = 0;
    return FALSE;
  }
...
BOOL LASreaderTXT::parse_attribute(const char* l, I32 index)
{
  ...
  if (header.attributes[index].data_type == 1)
  {
    ...
    {
      throw std::runtime_error(std::string("WARNING: attribute %d of type U8 is %d. clamped to [%d %d] range.")); //index, temp_i, U8_MIN, U8_MAX
  ...
  else if (header.attributes[index].data_type == 2)
  {
    ...
    {
      throw std::runtime_error(std::string("WARNING: attribute %d of type I8 is %d. clamped to [%d %d] range.")); //index, temp_i, I8_MIN, I8_MAX
  ...
  else if (header.attributes[index].data_type == 3)
  {
    ...
    {
      throw std::runtime_error(std::string("WARNING: attribute %d of type U16 is %d. clamped to [%d %d] range.")); //index, temp_i, U16_MIN, U16_MAX
  ...
  else if (header.attributes[index].data_type == 4)
  {
    ...
    {
      throw std::runtime_error(std::string("WARNING: attribute %d of type I16 is %d. clamped to [%d %d] range.")); //index, temp_i, I16_MIN, I16_MAX
  ...
  {
    throw std::runtime_error(std::string("WARNING: attribute %d not (yet) implemented.")); //index
    return FALSE;
  }
  return TRUE;
}
...
BOOL LASreaderTXT::parse(const char* parse_string)
{
  ...
  while (p[0])
  {
    ...
    else if (p[0] == 'i') // we expect the intensity
    {
      ...
      if (temp_f < 0.0f || temp_f >= 65535.5f) throw std::runtime_error(std::string("WARNING: intensity %g is out of range of unsigned short")); //temp_f
    ...
    else if (p[0] == 'a') // we expect the scan angle
    {
      ...
      if (temp_f < -128.0f || temp_f > 127.0f) throw std::runtime_error(std::string("WARNING: scan angle %g is out of range of char")); //temp_f
    ...
    else if (p[0] == 'n') // we expect the number of returns of given pulse
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 7) throw std::runtime_error(std::string("WARNING: return number %d is out of range of three bits")); //temp_i
    ...
    else if (p[0] == 'E') // we expect a terrasolid echo encoding)
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 3) throw std::runtime_error(std::string("WARNING: terrasolid echo encoding %d is out of range of 0 to 3")); //temp_i
    ...
    else if (p[0] == 'c') // we expect the classification
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 255) throw std::runtime_error(std::string("WARNING: classification %d is out of range of unsigned char")); //temp_i
    ...
    else if (p[0] == 'u') // we expect the user data
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 255) throw std::runtime_error(std::string("WARNING: user data %d is out of range of unsigned char")); //temp_i
    ...
    else if (p[0] == 'p') // we expect the point source ID
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 65535) throw std::runtime_error(std::string("WARNING: point source ID %d is out of range of unsigned short")); //temp_i
    ...
    else if (p[0] == 'e') // we expect the edge of flight line flag
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 1) throw std::runtime_error(std::string("WARNING: edge of flight line flag %d is out of range of boolean flag")); //temp_i
    ...
    else if (p[0] == 'd') // we expect the direction of scan flag
    {
      ...
      if (l[0] == 0) return FALSE;
      if (sscanf(l, "%d", &temp_i) != 1) return FALSE;
      if (temp_i < 0 || temp_i > 1) throw std::runtime_error(std::string("WARNING: direction of scan flag %d is out of range of boolean flag")); //temp_i
    ...
    {
      throw std::runtime_error(std::string("ERROR: unknown symbol '%c' in parse string")); //p[0]
    }
    p++;
  }
  return TRUE;
  ...
  while (p[0])
  {
        ...
        (p[0] != 'I'))   // we expect a hexadecimal coded intensity
    {
      if (p[0] >= '0' && p[0] <= '9')
      {
        ...
        if (index >= header.number_attributes)
        {
          throw std::runtime_error(std::string("ERROR: extra bytes attribute '%d' was not described.")); //index
          return FALSE;
        }
      ...
      {
        throw std::runtime_error(std::string("ERROR: unknown symbol '%c' in parse string. valid are")); //p[0]
