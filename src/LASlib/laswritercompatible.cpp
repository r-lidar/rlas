    ...
    if (header->evlrs)
    {
      for (int i = 0; i < (int)header->number_of_extended_variable_length_records; i++)
      {
        if (moveEVLRtoVLR)
        {
          ...
          {
#ifdef _WIN32
            throw std::runtime_error(std::string("large EVLR with user ID '%s' and record ID %d with payload size %I64d not moved to VLRs.")); //header->evlrs[i].user_id, header->evlrs[i].record_id, header->evlrs[i].record_length_after_header
#else
            throw std::runtime_error(std::string("large EVLR with user ID '%s' and record ID %d with payload size %lld not moved to VLRs.")); //header->evlrs[i].user_id, header->evlrs[i].record_id, header->evlrs[i].record_length_after_header
#endif
          }
        }
        else if (strcmp(header->evlrs[i].user_id, "LASF_Projection") == 0)
        {
          ...
          {
            throw std::runtime_error(std::string("unknown LASF_Projection EVLR with record ID %d not moved to VLRs.")); //header->evlrs[i].record_id
          }
        }
      }
    }
...
BOOL LASwriterCompatibleUp::open(LASheader* header, LASwriteOpener* laswriteopener)
{
  ...
  {
    throw std::runtime_error(std::string("ERROR: no compatibility VLR in header"));
    return FALSE;
  }
  // the compatibility VLR must have the right length
  if (compatibility_vlr->record_length_after_header != (2+2+4+148))
  {
    throw std::runtime_error(std::string("ERROR: compatibility VLR has %u instead of %u bytes in payload")); //compatibility_vlr->record_length_after_header, 2+2+4+148
    return FALSE;
  }
  I32 index_scan_angle = header->get_attribute_index("LAS 1.4 scan angle");
  if (index_scan_angle == -1)
  {
    throw std::runtime_error(std::string("ERROR: attribute \"LAS 1.4 scan angle\" is not in EXTRA_BYTES"));
    return FALSE;
  }
  start_scan_angle = header->get_attribute_start(index_scan_angle);
  ...
  {
    throw std::runtime_error(std::string("ERROR: attribute \"LAS 1.4 extended returns\" is not in EXTRA_BYTES"));
    return FALSE;
  }
  start_extended_returns = header->get_attribute_start(index_extended_returns);
  ...
  {
    throw std::runtime_error(std::string("ERROR: attribute \"LAS 1.4 classification\" is not in EXTRA_BYTES"));
    return FALSE;
  }
  start_classification = header->get_attribute_start(index_classification);
  ...
  if (index_scan_angle == -1)
  {
    throw std::runtime_error(std::string("ERROR: attribute \"LAS 1.4 flags and channel\" is not in EXTRA_BYTES"));
    return FALSE;
  }
  start_flags_and_channel = header->get_attribute_start(index_flags_and_channel);
  ...
  {
    throw std::runtime_error(std::string("ERROR: compatibility mode version %u not implemented")); //compatible_version
    return FALSE;
  }
  U32 unused;
  ...
  if (unused != 0)
  {
    throw std::runtime_error(std::string("WARNING: unused is %u instead of 0")); //unused
  }
